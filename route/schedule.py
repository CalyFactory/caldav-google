...
class Schedule(MethodView):
	def get(self,action):
		if action == 'getCalendarList':
			...
			print(mail)
			result = db_manager.query(
							"SELECT * FROM (user LEFT JOIN calendarList ON user.mail = calendarList.mail ) LEFT JOIN events ON calendarList.calendar_id = events.calendar_id WHERE user.mail = %s",(mail,)
						)
			rows = utils.fetch_all_json(result)
			print(rows)
